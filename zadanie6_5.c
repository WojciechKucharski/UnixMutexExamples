...
//PAMIĘĆ WSPÓLNA
	...
	shmid = shmget(421197, 2*sizeof(struct WSP), 0666 | IPC_CREAT);
...
// OBSŁUGA MUTEXA
		...
		pthread_mutex_init(&(WSPOFERTY->muteks), &(WSPOFERTY->at));
...
//PRZEBIEG LICYTACJI
	...
	if(fork()==0)
	{
		...
		{
			pthread_mutex_lock(&(WSPOFERTY->muteks));	
			WSPOFERTY->OFERTY[rand()%N_ITEMS]+=NOMINAL_RAISE;
			pthread_mutex_unlock(&(WSPOFERTY->muteks));
		}
	exit(0);
	}
	}
